...
namespace Chess
{
	...
	public class Board:System.Object
	{
		...
		public Board()
		{
			int i,j;
			square=new int[8,8];
			square[0,0]=1;//Pretas castle
			square[1,0]=2;//Pretas knight
			square[2,0]=3;//Pretas bishop
			square[3,0]=4;//Pretas queen
			square[4,0]=5;//Pretas king
			square[5,0]=3;//Pretas bishop
			square[6,0]=2;//Pretas knight
			square[7,0]=1;//Pretas castle
			for(i=0;i<8;i++)
			{
				square[i,1]=6;//Pretas pawns
				square[i,6]=12;//Brancas pawns
			}
			square[0,7]=7;//Brancas castle
			square[1,7]=8;//Brancas knight
			square[2,7]=9;//Brancas bishop
			square[3,7]=10;//Brancas queen
			square[4,7]=11;//Brancas king
			square[5,7]=9;//Brancas bishop
			square[6,7]=8;//Brancas knight
			square[7,7]=7;//Brancas castle
			for(i=0;i<8;i++)
				for(j=2;j<6;j++)
					square[i,j]=0;//Empty
		}
		...
		{
			if( (i + j) % 2 == 0)
				return 2;//back color of the board is Brancas
			else 
				return 1;//back color of the board is Pretas
		}
